...
注意：看板依赖两个插件同时在位——`/cost` 来自本插件，`/dshm` 来自 `@chengchengzhao/dsh-session-manager`。
...
发布前置：① ✅ LICENSE = **Apache-2.0**（已附全文，含专利授权，保留署名）；② ⏳ npm 账号登录（`npm login`，当前机器未登录）；③ ⏳ 真机装进 profile 验证（`dsh plugin --profile web add @chengchengzhao/dsh-cost-manager` + 重启）。
