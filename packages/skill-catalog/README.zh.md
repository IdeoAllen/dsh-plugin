...
DSH 第三个自定义插件 —— **技能总览**。与 `@chengchengzhao/dsh-cost-manager`（费用与余额）、
`@chengchengzhao/dsh-session-manager`（会话管理）出自同一套写法，界面与配色保持一致。
...
```json
"dependencies": {
  "@chengchengzhao/dsh-skill-catalog": "link:C:/Users/43594/Desktop/harness中心/工具/skill-catalog/dsh-plugin"
},
"dsh": { "profile": { "bundles": [ "...", "@chengchengzhao/dsh-skill-catalog" ] } }
```

`node_modules/@chengchengzhao/dsh-skill-catalog` 需为 **Junction** 指向该目录（与另两个 `@chengchengzhao` 插件同款）。
