...
| --- | --- | --- |
| [`@dachengge/dsh-cost-manager`](packages/cost-manager) | Balances, spend, burn rate, model price alignment and the anti-death floor. Registers 4 agent-callable tools. | Settings panel · `/cost` · `/board` |
| [`@dachengge/dsh-session-manager`](packages/session-manager) | Session browsing, cross-session full-text search, per-step token trends, export and archive. Folds the standalone dshm app onto the DSH port. | Settings panel · `/dshm` |
| [`@dachengge/dsh-skill-catalog`](packages/skill-catalog) | Skill catalog: what each skill is, how to use it, its trigger conditions, boundary, related models and verification level. | Settings panel · `/skills` · ops-board tab |
...
dsh plugin --profile web add @dachengge/dsh-cost-manager
dsh plugin --profile web add @dachengge/dsh-session-manager
dsh plugin --profile web add @dachengge/dsh-skill-catalog
```
